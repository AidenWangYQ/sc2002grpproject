...
import com.sc2002.battle.domain.Player;
import com.sc2002.battle.domain.Warrior;
import com.sc2002.battle.domain.Wizard;
import com.sc2002.battle.domain.Item;
import com.sc2002.battle.domain.Potion;
import com.sc2002.battle.domain.PowerStone;
import com.sc2002.battle.domain.SmokeBomb;
import com.sc2002.battle.domain.Inventory;
...
public class GameController {
    ...
    public void run() {
        ...
        while (playing) {
            ...
            promptItemSelection(items);
            Player  player = promptPlayerChoice(items);
    ...
    private Player promptPlayerChoice(List<Item> items) {
        System.out.println("  Choose your class (1 = Warrior, 2 = Wizard):");
        int choice = readIntInRange(1, 2);
        Player player = (choice == 1) ? new Warrior(new Inventory(items)) : new Wizard(new Inventory(items));
        System.out.printf("  → %s selected.%n%n", player.getName());
        return player;
    }
 
    private void promptItemSelection(List<Item> items) {
        String[] itemNames = { "Potion", "Power Stone", "Smoke Bomb" };
 
        System.out.println("  Choose your first item:");
        printItemMenu(itemNames);
        String firstName = itemNames[readIntInRange(1, 3) - 1];
        Item first = createItem(firstName);
        items.add(first);
 
        System.out.println("  Choose your second item (duplicates allowed):");
        printItemMenu(itemNames);
        String secondName = itemNames[readIntInRange(1, 3) - 1];
        Item second = createItem(secondName);
        items.add(second);
